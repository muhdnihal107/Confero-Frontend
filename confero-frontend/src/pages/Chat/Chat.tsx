...
const Chat: React.FC = () => {
  ...
  if (friendsLoading || groupsLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-blue-950 to-indigo-950">
        <div className="text-white text-4xl font-extrabold animate-pulse drop-shadow-lg">Loading...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#030103a8] text-white flex flex-col overflow-hidden">
      <Header />
      <main className="flex-grow max-w-7xl mt-16 mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col lg:flex-row gap-8">
        {/* Sidebar */}
        <aside className="w-full lg:w-96 bg-gray-900/30 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-indigo-500/20">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 drop-shadow-md">
              Chats
            </h2>
            <button
              onClick={() => setShowGroupModal(true)}
              className="px-5 py-2 bg-gradient-to-r from-green-500 to-teal-500 rounded-full text-sm font-bold text-white hover:from-green-600 hover:to-teal-600 focus:ring-2 focus:ring-green-400 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
            >
              Create Group
            </button>
          </div>
          <div className="max-h-[calc(100vh-220px)] overflow-y-auto space-y-4 pr-3 scrollbar-thin scrollbar-thumb-indigo-400 scrollbar-track-gray-900/50">
            {/* Group Chats Section */}
            {groupChats.length > 0 && (
              <>
                <h3 className="text-xl font-semibold text-indigo-200 mt-6 mb-3 drop-shadow-sm">Group Chats</h3>
                {groupChats.map((group) => (
                  <button
                    key={group.id}
                    onClick={() => handleSelectGroup(group)}
                    className={`w-full flex items-center p-4 rounded-2xl transition-all duration-300 ${
                      currentGroup?.id === group.id
                        ? 'bg-[#4c9d4496] shadow-xl'
                        : 'bg-gray-800/40 hover:bg-gray-800/70'
                    } transform hover:scale-[1.03] active:scale-[0.97] border border-indigo-500/10`}
                    aria-label={`Chat with ${getGroupDisplayName(group)}`}
                  >
                    <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center mr-4 ring-2 ring-indigo-400/30">
                      <span className="text-xl font-bold text-white">
                        {getGroupDisplayName(group).charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div className="text-left flex-1">
                      <p className="text-lg font-semibold text-white">{getGroupDisplayName(group)}</p>
                      <p className="text-sm text-indigo-300 truncate">{`${group.participants.length} members`}</p>
                    </div>
                    <div className="w-3 h-3 bg-gradient-to-br from-green-400 to-teal-400 rounded-full shadow-glow" />
                  </button>
                ))}
              </>
            )}
            ...
            {friends.length > 0 && (
              <>
                <h3 className="text-xl font-semibold text-indigo-200 mt-6 mb-3 drop-shadow-sm">Friends</h3>
                {friends.map((friend) => (
                  <button
                    key={friend.user_id}
                    onClick={() => handleSelectFriend(friend)}
                    className="w-full flex items-center p-4 rounded-2xl transition-all duration-300 bg-gray-800/40 hover:bg-gray-800/70 transform hover:scale-[1.03] active:scale-[0.97] border border-indigo-500/10"
                    aria-label={`Chat with ${friend.username || getDisplayName(friend.email)}`}
                  >
                    {friend.profile_photo ? (
                      <img
                        src={`http://localhost:8000${friend.profile_photo}`}
                        alt={friend.username || friend.email}
                        className="w-14 h-14 rounded-full object-cover mr-4 ring-2 ring-indigo-500/40 shadow-md"
                      />
                    ) : (
                      <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center mr-4 ring-2 ring-indigo-400/30">
                        <span className="text-xl font-bold text-white">
                    ...
                    <div className="text-left flex-1">
                      ...
                      <p className="text-sm text-indigo-300 truncate">{friend.email}</p>
                    </div>
                    <div className="w-3 h-3 bg-gradient-to-br from-green-400 to-teal-400 rounded-full shadow-glow" />
                  </button>
                ))}
            ...
            {groupChats.length === 0 && oneOnOneChats.length === 0 && friends.length === 0 && (
              <p className="text-indigo-300 text-center text-base font-semibold drop-shadow-sm">No chats or friends yet.</p>
            )}
          </div>
        </aside>

        {/* Chat Section */}
        <section className="flex-grow bg-gray-900/30 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-indigo-500/20 flex flex-col">
          {currentGroup ? (
            <>
              {/* Chat Header */}
              <div className="flex items-center justify-between mb-8 border-b border-indigo-500/30 pb-4">
                <div className="flex items-center">
                  {currentGroup.is_group_chat ? (
                    <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center mr-4 ring-2 ring-indigo-400/40 shadow-md">
                    ...
                    <img
                      src={`http://localhost:8000${getFriendDetails()?.profile_photo}`}
                      alt={getFriendDetails()?.username || getFriendDetails()?.email}
                      className="w-16 h-16 rounded-full object-cover mr-4 ring-2 ring-indigo-500/40 shadow-md"
                    />
                  ) : (
                    <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center mr-4 ring-2 ring-indigo-400/40 shadow-md">
                      <span className="text-2xl font-bold text-white">
                  ...
                  <div>
                    ...
                    {currentGroup.is_group_chat && (
                      <p className="text-sm text-indigo-300">{`${currentGroup.participants.length} members`}</p>
                    )}
                  </div>
                </div>
                {currentGroup.is_group_chat && (
                  <button
                    onClick={() => setShowGroupDetails(true)}
                    className="px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full text-sm font-bold text-white hover:from-indigo-600 hover:to-purple-600 focus:ring-2 focus:ring-indigo-400 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                  >
                    Group Details
                  </button>
                )}
              ...
              {/* Messages Area */}
              <div className="flex-grow overflow-y-auto mb-8 max-h-[calc(100vh-360px)] bg-gray-800/20 rounded-2xl p-6 scrollbar-thin scrollbar-thumb-indigo-400 scrollbar-track-gray-900/50">
                {messagesLoading ? (
                  <p className="text-indigo-300 text-center text-base animate-pulse drop-shadow-sm">Loading messages...</p>
                ) : messages.length === 0 ? (
                  <p className="text-indigo-300 text-center text-base font-semibold drop-shadow-sm">Start the conversation!</p>
                ) : (
                  messages.map((msg) => (
                    <div
                      key={msg.id}
                    ...
                    >
                      <div
                        className={`max-w-xs sm:max-w-md p-3 rounded-2xl shadow-lg transition-all duration-300 ${
                          msg.sender_email === user?.email
                            ? 'bg-[#29583396] text-white'
                            : 'bg-gray-800/50 text-white'
                        } hover:shadow-xl relative group border border-indigo-500/10`}
                      >
                        {currentGroup.is_group_chat && msg.sender_email !== user?.email && (
                        ...
                        {msg.message_type === 'text' && <p className="text-base leading-relaxed">{msg.content}</p>}
                        {msg.message_type === 'image' && (
                          <img
                            src={`http://localhost:8002${msg.media_file}`}
                            alt="Chat image"
                            className="w-72 h-auto rounded-lg shadow-md transition-transform duration-300 group-hover:scale-105 border border-indigo-500/20"
                          />
                        )}
                        {msg.message_type === 'video' && (
                          <video controls className="max-w-full rounded-lg shadow-md border border-indigo-500/20">
                            <source
                              src={`http://localhost:8002${msg.media_file}`}
                              type={msg.media_file?.endsWith('.mp4') ? 'video/mp4' : 'video/quicktime'}
                            />
                        ...
                        <p className="text-xs text-indigo-300/80 mt-2 text-right">
                          {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                    </div>
              ...
              {/* Message Input */}
              <div className="sticky bottom-0 flex items-center gap-3 bg-gray-900/40 backdrop-blur-xl rounded-full p-3 border border-indigo-500/20 shadow-lg">
                <input
                  type="text"
                  value={messageText}
                  onChange={(e) => setMessageText(e.target.value)}
                  placeholder="Type a message..."
                  className="flex-grow py-3 px-5 bg-gray-800/30 text-white placeholder-indigo-300/50 text-base rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-300 shadow-inner"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      handleSendMessage();
                ...
                <label
                  htmlFor="file-upload"
                  className="p-3 bg-gray-800/50 rounded-full hover:bg-gray-700/70 cursor-pointer transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-110"
                  aria-label="Upload file"
                >
                ...
                <button
                  onClick={handleSendMessage}
                  className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white py-3 px-8 rounded-full hover:from-indigo-600 hover:to-purple-600 transition-all duration-300 text-base font-bold shadow-md disabled:opacity-50 transform hover:scale-105 active:scale-95 hover:shadow-xl"
                  disabled={isSending}
                  aria-label="Send message"
                >
                  Send
            ...
            <div className="flex-grow flex items-center justify-center">
              <p className="text-indigo-300 text-xl font-semibold animate-pulse drop-shadow-md">Select a chat to start messaging</p>
            </div>
          )}
        </section>
      </main>
...
export default Chat;
    ...
    // {oneOnOneChats.length > 0 && (
    //   <>
    //     <h3 className="text-xl font-semibold text-indigo-200 mt-6 mb-3 drop-shadow-sm">1:1 Chats</h3>
    //     {oneOnOneChats.map((group) => (
    //       <button
    //         key={group.id}
    //         onClick={() => handleSelectGroup(group)}
    ...
    //         <div className="text-left flex-1">
    //           <p className="text-lg font-semibold text-white">{getGroupDisplayName(group)}</p>
    //           <p className="text-sm text-indigo-300 truncate"> Chat</p>
    //         </div>
    //         <div className="w-3 h-3 bg-gradient-to-br from-green-400 to-teal-400 rounded-full shadow-glow" />
    //       </button>
    //     ))}
